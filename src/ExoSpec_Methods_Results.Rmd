...
#### LOAD DATA ####
ExoSpec <- readRDS(here("output/data_out/ExoSpec_Cohort.RDS"))
ExoSpecMets <- readRDS(here("data/ExoMeth_Mets.RDS"))
...
NanoGenes <- ExoSpec %>% 
   select(-Sample_ID, -Gleason,
          -starts_with("e", ignore.case = FALSE), 
          -all_of(outcomes), -all_of(ClinVars)) %>% 
   names

#Grab the names of all the peptides, they all start with lowercase "e" so relatively easy
Peptides <- ExoSpec %>% 
...
```{r RF Models}
# Using the LASSO-selected features, fit Random Forest models to the datasets:
...
ExoSpecMets <- ExoSpecMets %>%
   select(-Sample_ID, -Cat, -DRESize) %>%
   select(ConfirmedVariables$ExoSpec$Vars)

#Produce the "positive control" predictions from the Mets samples
...
Linked cf-RNA and proteomic data were available for `r nrow(ExoSpec)` patients within the Movember GAP1 cohort, all originating from the NNUH and forming the ExoSpec development cohort (Table 1). The proportion of Gleason ≥7 disease in the ExoSpec cohort was `r round(sum(ExoSpec$LowGSig == "Yes") / nrow(ExoSpec) * 100,)`%. 
...
**Table 2.** *Features selected by the cross-validated LASSO to be used as input variables for each Random Forest comparator model. Variables selected for the fully integrated model (ExoSpec) are in the highlighted column; for example; SNORA20 is selected within the ExoRNA model, but not in ExoSpec.*
```{r Features selected}
...
```{r Little stats}
# Requested stats for Table 1 p values...
smolstats <- ExoSpec %>% 
      select(is_C, Age, PSA) 

wilcox.test(smolstats %>% filter(is_C == "Yes") %>% pull(Age),
            smolstats %>% filter(is_C == "No") %>% pull(Age))

wilcox.test(smolstats %>% filter(is_C == "Yes") %>% pull(PSA),
            smolstats %>% filter(is_C == "No") %>% pull(PSA))

#Calculate the fold changes between cancer and non-cancer patients the variables:
EveryVar <- bind_rows(ConfirmedVariables) %>% 
   pull(Vars) %>% 
   unique()

LowGsig_FoldChange <- ExoSpec %>% 
   select(LowGSig, all_of(EveryVar)) %>% 
   group_by(LowGSig) %>% 
   summarise_all(mean) %>% 
   pivot_longer(all_of(EveryVar), names_to = "Variable", values_to = "Count") %>% 
   pivot_wider(names_from = LowGSig, values_from = Count) %>% 
   mutate('Change from No' = Yes - No)

Cancer_Fold_Change <- ExoSpec %>% 
   select(is_C, all_of(EveryVar)) %>% 
   group_by(is_C) %>% 
   summarise_all(mean) %>% 
   pivot_longer(all_of(EveryVar), names_to = "Variable", values_to = "Count") %>% 
   pivot_wider(names_from = is_C, values_from = Count) %>% 
   mutate('Change from No' = Yes - No)

write_csv(LowGsig_FoldChange, "output/data_out/LowGleasonFoldChanges.csv")
write_csv(Cancer_Fold_Change, "output/data_out/CancerFoldChanges.csv")
```