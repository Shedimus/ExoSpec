...
**MassSpec methods go here**. Peptide data were filtered *a priori* by only retaining peptides quantified at any level in at least 30% of either cancer or non-cancer samples.
...
Following *a priori* filtering, the ExoSpec cohort had a total of `r length(c(NanoGenes, Peptides, ClinVars))` possible variables for modelling (cf-RNA (*n* = `r length(NanoGenes)`), methylation (*n* = `r length(Peptides)`) and clinical variables (*n* = `r length(ClinVars)`) and made feature selection a key task for minimising the potential for model overfit and increasing the robustness of any trained models. 

Variables with a significant association with the modified Gleason label described below were robustly identified by means of a 20-fold cross-validated LASSO-penalised regression model, fit using the *glmnet* package [@glmnet]. 
...
As ExoSpec Risk Score (range 0-1) increased, the likelihood of high-grade disease being detected on biopsy was significantly greater (`r OddsRatio`; ordinal logistic regression, **Figure 2**). 

**Table 2.** *AUC of all trained models for detecting outcomes of an initial biopsy for varying clinically significant thresholds. Brackets show 95% confidence intervals of the AUC, calculated from 1,000 stratified bootstrap resamples. Input variables for each model are detailed in Table 1.*
```{r model AUCs}
...
ExoSpec was superior to the standard of care model for predicting Gleason ≥3+4, returning an AUC of `r str_replace(AUC_Table$ExoSpec[2], "\\(", "\\(95% CI: ")`, *p* = `r round(LowGBoot$p.value, 3)` but was not significantly better than standards of care in predicting Gleason ≥4+3 disease (`r str_replace(AUC_Table$ExoSpec[1], "\\(", "\\(95% CI: ")` *p* = `r round(GleaSigBoot$p.value, 3)`, **Table 3**).
...
Resampling of ExoSpec predictions via estimation plots allowed for comparisons of mean ExoSpec signatures between groups by 1,000 bias-corrected and accelerated bootstrap resamples (**Figure 4**). The mean ExoSpec differences between patients with no evidence of cancer on biopsy were: Gleason 6 = `r EstimationSummary$Text[2]`, Gleason 3+4 = `r EstimationSummary$Text[1]` and Gleason ≥4+3 = `r EstimationSummary$Text[3]`. Interestingly, patients with no evidence of cancer had a lower ExoSpec risk score (mean difference = `r EstimationSummary$Text[4]`) than those men with a raised PSA but negative for cancer on biopsy (**Supplementary Figure 5**).
...
DCA_SoC_Relative <- map(DCA_Plot_Data, function(OutcomeDCA){
   OutcomeDCA %>% 
      ...
      mutate(MassSpec = MassSpec - SoC,
             ...
             ExoSpec = ExoSpec - SoC) %>% 
      select(-SoC, -All) %>% 
      pivot_longer(cols = None:ExoSpec,
                   names_to = "Comparator",
                   values_to = "Net_Benefit") %>% 
      mutate(Comparator = factor(Comparator, 
                                 levels = c("None", "MassSpec", "ExoRNA", "ExoSpec"),
                                 ordered = TRUE))
})
...
DCAPlots <- map2(
   ...
   function(outcome, outcome_name) {
      ggplot(outcome, aes(x = Threshold, colour = Comparator, y = Net_Benefit)) +
         geom_line(size = 1.07) +
         theme_cowplot() +
         coord_cartesian(ylim = c(-0.05, 0.5), xlim = c(0, 0.6)) +
         labs(x = paste0("Decision threshold for detection of ", outcome_name),
              y = "Standardised Net Benefit\nrelative to Standard of Care",
              colour = "Comparators") +
         scale_color_manual(values = c(D3Orange, D3Green, D3Purp, D3Red),
                            labels = c("Standard of Care", "MassSpec", "ExoRNA", "ExoSpec")) +
         theme(
